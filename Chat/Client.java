...
public class Client implements ActionListener{
    ...
    Client() { // Client constructor
        ...
        ImageIcon i1 = new ImageIcon(getClass().getResource("3.png")); // image is loaded from the classpath using ClassLoader.getSystemResource() 
        ...
        f.setLocation(800, 50); // Sets the location of the window 
   ...
   public static void main(String[] args) {
        ...
        try {
            Socket s = new Socket("127.0.0.1", 6001); // creates a new socket (Socket object) that attempts to connect to a server running on the same localhost at the specified port.
            DataInputStream din = new DataInputStream(s.getInputStream()); //set up input and output streams (DataInputStream and DataOutputStream) for communication over the Socket s, which is established between a client and a server.
            dout = new DataOutputStream(s.getOutputStream()); 

            while(true){
                a1.setLayout(new BorderLayout());
                String msg = din.readUTF(); //used to read a UTF-8 encoded string from an input stream using a DataInputStream 
                JPanel panel = formatLabel(msg); //formats the message (msg) into a JPanel object
